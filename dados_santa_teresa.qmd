...
datatable(pagamentos %>% filter( funcao !="") %>% group_by(funcao) %>% summarise(valor = sum(valor, na.rm = FALSE)))
...
pagamentos <- pagamentos %>%   mutate(data_ts = ceiling_date(data %m-% months(0), 'month') %m-% days(1))  
...
pagamento_clean <- pagamentos %>% select(data, funcao_nome, grupo_cod, grupo_nome, elemento_cod, subfuncao_nome, elemento_nome, favorecido_nome, mes_ano,  data_ts ,ano, valor)
 library(openxlsx)
